...
_SCOOP_STL_PATH = _ROOT / "ur_urdf" / "newscoop.stl"
...
class Example:
    def __init__(self, viewer, args):
        ...
        # air_drag lives on the Config only (not on model.mpm), so the generic
        # loop above already handles it via setattr(mpm_options, 'air_drag', ...).
        # Explicitly confirm it here so the intent is clear.
        mpm_options.air_drag = args.air_drag
    ...
    def step(self):
        ...
        for i in range(self.sim_substeps):
            ...
            # After MPM step: catch any particles that tunnelled through the
            # scoop walls during the momentum solve.  Gap of 2×voxel_size
            # ensures particles approaching the scoop rim at particle_max_velocity
            # are found by the SDF query before they exit the far side.
    ...
    def create_parser():
        ...
        # Particle volume
        parser.add_argument("--emit-lo", type=float, nargs=3, default=[-0.15, -0.35, 0.03])
        parser.add_argument("--emit-hi", type=float, nargs=3, default=[ 0.15, -0.05, 0.05])
        ...
        parser.add_argument("--max-iterations", "-it", type=int, default=500)
        ...
        # Reference: Newton example_mpm_granular.py + Klar et al. 2016 SIGGRAPH
        # young_modulus=1e15 makes grains nearly incompressible (grains don't compress internally).
        # yield_pressure=1e12 keeps material in elastic regime; pile shape comes from friction only.
        ...
        parser.add_argument("--young-modulus",      "-ym",  type=float, default=1e9)
        ...
        parser.add_argument("--yield-pressure",     "-yp",  type=float, default=1e12)
